        ...
        ServiceFactory.getProvideHttpService().getLogin(requestLogin)
                ...
                .subscribe(new Action1<LoginInBean.ResultsBean>() {
                    ...
                    public void call(LoginInBean.ResultsBean resultsBean) {
                        ...
                        RxBus.getInstance().send(Events.EVENTS_MEMBER_USER_LOGIN, new Object());//发送events事件会员状态
                        RxBus.getInstance().send(Events.EVENTS_SUBMIT_ORDER_USER_LOGIN, new Object());
                        finish();
                    }
                }, new Action1<Throwable>() {
                    @Override
